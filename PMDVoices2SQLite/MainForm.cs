...
namespace PMDVoices2SQLite
{
    public partial class MainForm : Form
    {
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            TextBoxLogger.Log($"バージョン {buildDateTime}");
            TextBoxLogger.Log($"起動 {DateTime.Now}");
#if DEBUG
            this.comboBoxMMLPath.Text = @"E:\Google ドライブ\東方旧作用ツール\東方旧作スターターパッケージ\東方旧作MMLデータ\zun_AMS4To1.mml";
            ...
            this.comboBoxMMLPath.Items.Add(@"E:\Google ドライブ\東方旧作用ツール\東方旧作スターターパッケージ\東方旧作MMLデータ\zun.mml");
            this.comboBoxMMLPath.Items.Add(@"E:\Google ドライブ\東方旧作用ツール\東方旧作スターターパッケージ\東方旧作MMLデータ\zun_AMS4To1.mml");
            this.comboBoxMMLPath.Items.Add(@"E:\Google ドライブ\東方旧作用ツール\東方旧作スターターパッケージ\東方旧作MMLデータ\th02_PMD\02_10.txt");
        ...
        private void buttonMMLPath_Click(object sender, EventArgs e)
        {
            ...
            using (OpenFileDialog dlg = new())
            {
                ...
                dlg.Filter = "MMLファイル(*.mml)|*.mml|テキストファイル(*.txt)|*.txt|すべてのファイル(*.*)|*.*";
                //ダイアログを開く
        ...
        private void buttonDBPath_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog dlg = new())
            {
                ...
                dlg.Filter = "SQLite データベースファイル(*.db *.sqlite *.sqlite3 *.db3)|*.db;*.sqlite;*.sqlite3;*.db3|すべてのファイル(*.*)|*.*";
...
#pragma warning restore CS8604 // Null 参照引数の可能性があります。
        ...
        public string ReadMML()
        {
            ...
            TextBoxLogger.Log($"MML読み込み完了 Path:{this.comboBoxMMLPath.Text}");
            TextBoxLogger.Log($"MMLの内容:{Environment.NewLine}{mmlText}");
        ...
        public static async Task<IEnumerable<PMDVoice>?> SearchFromSQLite(IEnumerable<PMDVoice> voices, string dbPath, bool optionNearVoice = true)
        {
            ...
            foreach (PMDVoice voice in voices)
            {
                ...
                TextBoxLogger.Log($"-----------------------------------------");
                TextBoxLogger.Log($"検索結果　{resultSearch.Count} 件");
                var first = resultSearch.FirstOrDefault();
                if (first is not null)
                {
                    TextBoxLogger.Log($"合致音色： {Environment.NewLine}" +
                        $"{first.ToMML(is3digits: true)} {Environment.NewLine}" +
                    $"MMLファイル名： {first.MMLFileName} ");
                ...
                else if (optionNearVoice)
                {
                    ...
                    if (minDistance is not null)
                    {
                        ...
                        TextBoxLogger.Log($"距離： {minDistance.CoreDistance(voice)}");
                        TextBoxLogger.Log($"最も近い音色： {Environment.NewLine}{minDistance.ToMML(is3digits: true)}");
                        TextBoxLogger.Log($"MMLファイル名： {minDistance.MMLFileName} ");
                    }
                }
                TextBoxLogger.Log($"検索音色　{Environment.NewLine}{voice.ToMML(is3digits: true)} ");
            }
            TextBoxLogger.Log("検索終了");
            return returnVoices;
        }
